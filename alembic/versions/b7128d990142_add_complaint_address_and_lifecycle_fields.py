...
down_revision: Union[str, None] = '9a011ef9321c'
...
def upgrade() -> None:
    ...
    op.add_column('complaints', sa.Column('filling_on_behalf_of', sa.Boolean(), server_default='false', nullable=False))
    op.add_column('complaints', sa.Column('timestamp', sa.DateTime(), server_default=sa.text('now()'), nullable=False))
    op.add_column('complaints', sa.Column('closing_time_stamp', sa.DateTime(), nullable=True))
    ...
    op.create_index(op.f('ix_complaint_address_complaint_id'), 'complaint_address', ['complaint_id'], unique=True)
    op.create_index(op.f('ix_complaint_address_id'), 'complaint_address', ['id'], unique=False)


def downgrade() -> None:
    op.drop_index(op.f('ix_complaint_address_id'), table_name='complaint_address')
    op.drop_index(op.f('ix_complaint_address_complaint_id'), table_name='complaint_address')
