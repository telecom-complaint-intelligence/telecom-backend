...
def send_email_otp(to_email: str, otp: str) -> bool:
    ...
    if not (smtp_host and smtp_port and smtp_user and smtp_pass):
        print("[EMAIL SYSTEM WARNING] SMTP configuration variables not found in backend .env. Skipping real email delivery.")
        return False
    ...
    try:
        ...
        server.close()
        print(f"[EMAIL SYSTEM SUCCESS] Real verification email successfully sent to {to_email}")
        return True
...
def build_user_response(db_user: User):
    ...
    profile_data = {
        "name": None, "phone": None, "profilePicture": None,
        "address": None, "city": None, "stateVal": None,
        "country": "India", "zipcode": None
    }
    service_data = {
        "accountRef": None, "billCycle": None, "activePlan": None, "connectionStatus": None, "planUsage": None
    }
...
async def google_login(request: GoogleLoginRequest, db: Annotated[Session, Depends(get_db)]):
    email = None
    name = "Telu User"
    profile_pic = None
...
async def register_user(request: UserRegisterRequest, db: Annotated[Session, Depends(get_db)]):
...
async def login_user(request: UserLoginRequest, db: Annotated[Session, Depends(get_db)]):
...
async def complete_profile(request: ProfileCompleteRequest, db: Annotated[Session, Depends(get_db)]):
    db_user = db.query(User).filter(User.email == request.email).first()
    if not db_user:
        raise HTTPException(
...
async def update_service_details(request: ServiceDetailsUpdateRequest, db: Annotated[Session, Depends(get_db)]):
    db_user = db.query(User).filter(User.email == request.email).first()
    if not db_user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def cookie_consent(request: CookieConsentRequest, db: Annotated[Session, Depends(get_db)]):
    db_user = db.query(User).filter(User.email == request.email).first()
    if not db_user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def resend_otp(request: ResendOtpRequest, db: Annotated[Session, Depends(get_db)]):
    db_user = db.query(User).filter(User.email == request.email).first()
    if not db_user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def verify_session(request: VerifySessionRequest, db: Annotated[Session, Depends(get_db)]):
    db_user = db.query(User).filter(User.email == request.email).first()
    if not db_user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
